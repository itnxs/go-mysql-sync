...
func newEventHandler(store BinLogStore, config *Config) (*eventHandler, error) {
	tables := make(map[string]interface{})
	for _, table := range config.Tables {
		table = strings.ToLower(fmt.Sprintf("%s.%s", config.driverConfig.DBName, table))
		tables[table] = table
	}
	...
	return &eventHandler{
		...
		onMessage: func(m Message) error {
			config.Logger.WithField("data", m).Info("onMessage")
			return nil
		},
	}, nil
}
